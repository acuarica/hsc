...
fromDecl DataDecl{} = []
fromDecl TypeDecl{} = []
fromDecl x = error $ "Unhandled fromDecl --here--: " ++ show x
...
fromAlt :: Alt -> (Pat, Exp)
fromAlt (Alt _ pat (UnGuardedRhs bod) (Just (BDecls []))) = (fromPat pat, fromExp bod)
...
toDecl :: (Var,Exp) -> Decl
toDecl (f,x) = PatBind sl (PVar $ Ident f) (UnGuardedRhs $ toExp x) (Just (BDecls []))
...
toAlt :: (Pat, Exp) -> Alt
toAlt (x,y) = Alt sl (toPat x) (UnGuardedRhs $ toExp y) (Just (BDecls []))
